...
- **음성 시스템 명령** ([src/tasks/voice_commands.py](src/tasks/voice_commands.py)): `ambient.add_system_handler()`로 등록 — consumed 발화는 conversation_log/perception/일반 handler 전부 skip(agent 노출 X). 현재 지원: (1) **"디버그 모드"** → `nmcli connection up jhS26u`(폰 테더링), 실패 시 `WIFI_FALLBACK_CHAIN`(bbang5G, SG_Open) 순차. FOCUSED→HAPPY/CONTENT/WORRIED. 30s cooldown. (2) **"셧다운"/"전원 꺼"/"shutdown"** → systemctl poweroff. confirm 패턴: 첫 발화로 10초 pending(WORRIED + 카운트), 둘째 발화로 실행. "취소"/"아니야"/"끄지마" 또는 timeout 시 해제. 단독 "꺼"는 false positive 위험으로 제외. (3) **"날씨 알려줘"/"날씨 어때"/"오늘 날씨"** → `WeatherClient.snapshot().one_liner()`(예: "분당 맑음 31°C · 습도 21%") LCD에 8초 표시 + CONTENT. 30분 캐시 hit면 즉시. 키 없으면 WORRIED. 5s cooldown(LCD 깜빡 방지). 권한: polkit rule 두 개 — `50-nm-roboface.rules`(NetworkManager) + `50-shutdown-roboface.rules`(login1 power-off). repo 외부 설치 (robot setup 한 번만)
